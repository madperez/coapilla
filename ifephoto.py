...
# -*- coding: utf-8 -*-
"""
Created on Fri Dec 18 12:51:53 2020
...
def setup_database():
    ...
    return database
def setup_database_ife():
    """
    Load reference images and create a database of their face encodings
    """
    database = {}

    for filename in glob.glob(os.path.join(IFE_PATH, '*.jpg')):
        # load image
        image_rgb = face_recognition.load_image_file(filename)
...
def run_face_recognition_ife(database):
    """
    Start the face recognition via the webcam
    """
    # Open a handler for the camera
...
run_face_recognition_ife(database)
